...
export class AuthUserUseCase implements MakeAuth {
  ...
  async auth(cpf: string, password: string): Promise<Account> {
    try {
      ...
      const isValidPassword : boolean = await this.bcrypt.compare(password, user?.password as string);

      if (!isValidPassword || !user) {
        throw new CredentialsInvalidError('Invalid cpf or password', HttpStatus.Unauthorized);
      }
